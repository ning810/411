...
<div class="header">
  <div class="logo"><div class="logo-icon">📦</div><span>仓库管家 · 进销存</span></div>
  <div class="header-actions">
    <button class="btn" onclick="exportData()">📤 导出</button>
    <button class="btn" onclick="importData()">📥 导入</button>
    <button class="btn btn-primary" onclick="openItemModal()">＋ 新增商品</button>
  </div>
</div>
...
<div class="sidebar">
  ...
  <div class="nav-item active" onclick="go('dashboard')" data-page="dashboard"><span class="icon">📊</span>仪表盘</div>
  <div class="nav-section">进销存</div>
  <div class="nav-item" onclick="go('inventory')" data-page="inventory"><span class="icon">📋</span>商品库存</div>
  <div class="nav-item" onclick="go('purchase')" data-page="purchase"><span class="icon">🛒</span>采购管理</div>
  <div class="nav-item" onclick="go('sales')" data-page="sales"><span class="icon">💰</span>销售管理</div>
  <div class="nav-item" onclick="go('inout')" data-page="inout"><span class="icon">🔄</span>快速出入库</div>
  <div class="nav-section">数据</div>
  <div class="nav-item" onclick="go('suppliers')" data-page="suppliers"><span class="icon">🏭</span>供应商</div>
  <div class="nav-item" onclick="go('customers')" data-page="customers"><span class="icon">👥</span>客户管理</div>
  <div class="nav-item" onclick="go('stocktake')" data-page="stocktake"><span class="icon">📝</span>库存盘点</div>
  <div class="nav-item" onclick="go('finance')" data-page="finance"><span class="icon">📈</span>财务报表</div>
  <div class="nav-item" onclick="go('log')" data-page="log"><span class="icon">🕐</span>操作记录</div>
  <div class="nav-section">设置</div>
  <div class="nav-item" onclick="go('categories')" data-page="categories"><span class="icon">🏷️</span>分类管理</div>
</div>
...
<div class="page active" id="page-dashboard">
  ...
  <div class="table-wrap">
    <div class="table-header">⚠️ 低库存预警</div>
    <table><thead><tr><th>商品</th><th>当前库存</th><th>预警值</th><th>操作</th></tr></thead>
    <tbody id="low-body"></tbody></table>
    <div class="empty-state" id="low-empty" style="display:none;padding:24px;"><div class="msg">✅ 所有商品库存充足</div></div>
  </div>
</div>

<!-- ====== INVENTORY ====== -->
<div class="page" id="page-inventory">
  <div class="page-title">商品库存</div>
  <div class="toolbar">
    <div class="search-box"><span class="si">🔍</span><input id="inv-search" placeholder="搜索商品..." oninput="renderInv()"></div>
    <select id="inv-cat" onchange="renderInv()"><option value="">全部分类</option></select>
    <select id="inv-status" onchange="renderInv()"><option value="">全部状态</option><option value="ok">充足</option><option value="low">偏低</option><option value="out">缺货</option></select>
    <button class="btn btn-primary" onclick="openItemModal()">＋ 新增</button>
  </div>
  <div class="table-wrap">
    <table><thead><tr><th onclick="sortInv('name')">名称</th><th onclick="sortInv('sku')">编号</th><th>分类</th><th onclick="sortInv('quantity')">库存</th><th onclick="sortInv('costPrice')">成本价</th><th onclick="sortInv('salePrice')">售价</th><th>状态</th><th>操作</th></tr></thead>
    <tbody id="inv-body"></tbody></table>
    <div class="empty-state" id="inv-empty" style="display:none;"><div class="icon">📦</div><div class="msg">暂无商品</div></div>
  </div>
</div>
...
<div class="page" id="page-purchase">
  ...
  <div class="toolbar">
    <div class="search-box"><span class="si">🔍</span><input id="po-search" placeholder="搜索采购单..." oninput="renderPO()"></div>
    <select id="po-status" onchange="renderPO()"><option value="">全部状态</option><option value="pending">待入库</option><option value="done">已完成</option><option value="cancel">已取消</option></select>
    <button class="btn btn-primary" onclick="openPOModal()">＋ 新建采购单</button>
  </div>
  <div class="table-wrap">
    <table><thead><tr><th>采购单号</th><th>供应商</th><th>商品</th><th>数量</th><th>采购金额</th><th>状态</th><th>日期</th><th>操作</th></tr></thead>
    <tbody id="po-body"></tbody></table>
    <div class="empty-state" id="po-empty" style="display:none;"><div class="icon">🛒</div><div class="msg">暂无采购记录</div></div>
  </div>
</div>
...
<div class="page" id="page-sales">
  ...
  <div class="toolbar">
    <div class="search-box"><span class="si">🔍</span><input id="so-search" placeholder="搜索销售单..." oninput="renderSO()"></div>
    <select id="so-status" onchange="renderSO()"><option value="">全部状态</option><option value="pending">待出库</option><option value="done">已完成</option><option value="cancel">已取消</option></select>
    <button class="btn btn-primary" onclick="openSOModal()">＋ 新建销售单</button>
  </div>
  <div class="table-wrap">
    <table><thead><tr><th>销售单号</th><th>客户</th><th>商品</th><th>数量</th><th>销售金额</th><th>利润</th><th>状态</th><th>日期</th><th>操作</th></tr></thead>
    <tbody id="so-body"></tbody></table>
    <div class="empty-state" id="so-empty" style="display:none;"><div class="icon">💰</div><div class="msg">暂无销售记录</div></div>
  </div>
</div>

<!-- ====== QUICK IN/OUT ====== -->
<div class="page" id="page-inout">
  <div class="page-title">快速出入库</div>
  <div class="table-wrap" style="padding:22px;max-width:560px;">
    <div class="form-group"><label class="form-label">选择商品</label><select id="io-item" class="form-input"></select></div>
    <div class="form-row">
      <div class="form-group"><label class="form-label">操作类型</label><select id="io-type" class="form-input"><option value="in">入库</option><option value="out">出库</option></select></div>
      <div class="form-group"><label class="form-label">数量</label><input type="number" id="io-qty" class="form-input" min="1" value="1"></div>
    </div>
    <div class="form-group"><label class="form-label">备注</label><input type="text" id="io-note" class="form-input" placeholder="备注信息..."></div>
    <button class="btn btn-primary" onclick="doIO()" style="margin-top:6px;">✅ 确认</button>
  </div>
</div>
...
<div class="page" id="page-suppliers">
  ...
  <div class="toolbar"><button class="btn btn-primary" onclick="openSupModal()">＋ 新增供应商</button></div>
  <div class="table-wrap">
    <table><thead><tr><th>名称</th><th>联系人</th><th>电话</th><th>地址</th><th>采购次数</th><th>采购总额</th><th>操作</th></tr></thead>
    <tbody id="sup-body"></tbody></table>
    <div class="empty-state" id="sup-empty" style="display:none;"><div class="icon">🏭</div><div class="msg">暂无供应商</div></div>
  </div>
</div>
...
<div class="page" id="page-customers">
  ...
  <div class="toolbar"><button class="btn btn-primary" onclick="openCustModal()">＋ 新增客户</button></div>
  <div class="table-wrap">
    <table><thead><tr><th>名称</th><th>联系人</th><th>电话</th><th>地址</th><th>购买次数</th><th>消费总额</th><th>操作</th></tr></thead>
    <tbody id="cust-body"></tbody></table>
    <div class="empty-state" id="cust-empty" style="display:none;"><div class="icon">👥</div><div class="msg">暂无客户</div></div>
  </div>
</div>
...
<div class="page" id="page-stocktake">
  ...
  <p style="color:var(--text2);font-size:13px;margin-bottom:16px;">核对系统库存与实际库存，如有差异可直接修正。</p>
  <div class="toolbar"><button class="btn btn-green" onclick="doStocktake()">✅ 提交盘点结果</button></div>
  <div class="table-wrap">
    <table><thead><tr><th>商品名称</th><th>编号</th><th>系统库存</th><th>实际库存</th><th>差异</th></tr></thead>
...
<div class="page" id="page-finance">
  ...
  <div class="table-wrap">
    <div class="table-header">📊 商品利润明细</div>
    <table><thead><tr><th>商品</th><th>销售数量</th><th>销售收入</th><th>采购成本</th><th>毛利润</th><th>利润率</th></tr></thead>
...
<div class="page" id="page-log">
  ...
  <div class="toolbar">
    <select id="log-filter" onchange="renderLog()"><option value="">全部</option><option value="in">入库</option><option value="out">出库</option><option value="purchase">采购</option><option value="sale">销售</option><option value="add">新增</option><option value="edit">编辑</option><option value="delete">删除</option><option value="stocktake">盘点</option></select>
    <button class="btn btn-danger btn-sm" onclick="clearLog()">清空</button>
  </div>
  <div class="table-wrap">
    <div id="log-list"></div>
    <div class="empty-state" id="log-empty" style="display:none;"><div class="icon">🕐</div><div class="msg">暂无记录</div></div>
  </div>
</div>
...
<div class="page" id="page-categories">
  ...
  <div class="toolbar">
    <input type="text" id="new-cat" class="form-input" placeholder="新分类名称" style="max-width:260px;">
    <button class="btn btn-primary" onclick="addCat()">＋ 添加</button>
  </div>
  <div class="table-wrap">
    <table><thead><tr><th>分类</th><th>商品数</th><th>操作</th></tr></thead>
...
<div class="mobile-nav">
  <button class="m-nav active" onclick="go('dashboard')" data-page="dashboard"><span class="icon">📊</span>概览</button>
  <button class="m-nav" onclick="go('inventory')" data-page="inventory"><span class="icon">📋</span>库存</button>
  <button class="m-nav" onclick="go('purchase')" data-page="purchase"><span class="icon">🛒</span>采购</button>
  <button class="m-nav" onclick="go('sales')" data-page="sales"><span class="icon">💰</span>销售</button>
  <button class="m-nav" onclick="go('finance')" data-page="finance"><span class="icon">📈</span>报表</button>
</div>

<!-- ====== GENERIC MODAL ====== -->
<div class="modal-overlay" id="modal"><div class="modal">
  <div class="modal-header"><div class="modal-title" id="m-title"></div><button class="modal-close" onclick="closeM()">✕</button></div>
...
// ===== DATA =====
const D = {
  items: [], purchases: [], sales: [], suppliers: [], customers: [], logs: [], categories: ['默认','电子产品','办公用品','食品','服装','其他']
};
...
function renderDash() {
  ...
  document.getElementById('dash-stats').innerHTML = [
    {l:'商品种类',v:totalItems,c:'var(--accent)'},
    {l:'总库存量',v:totalQty.toLocaleString(),c:'var(--text)'},
    {l:'库存价值',v:money(totalVal),c:'var(--green)'},
    {l:'低库存预警',v:lowCount,c:lowCount?'var(--red)':'var(--green)'},
    {l:'采购总额',v:money(totalPurchase),c:'var(--cyan)'},
    {l:'销售总额',v:money(totalSales),c:'var(--purple)'},
    {l:'总利润',v:money(totalProfit),c:totalProfit>=0?'var(--green)':'var(--red)'},
  ].map(s=>`<div class="stat-card"><div class="stat-label">${s.l}</div><div class="stat-value" style="color:${s.c}">${s.v}</div></div>`).join('');

  // Charts
  const catData = {};
  D.items.forEach(i => { catData[i.category||'未分类'] = (catData[i.category||'未分类']||0)+i.quantity; });
  ...
  document.getElementById('dash-charts').innerHTML = `
    <div class="chart-container"><div class="chart-title">📦 各分类库存量</div><div class="bar-chart">${Object.entries(catData).map(([k,v],i)=>`<div class="bar-col"><div class="bar-val">${v}</div><div class="bar" style="height:${v/maxQ*130}px;background:${colors[i%colors.length]}"></div><div class="bar-label">${esc(k)}</div></div>`).join('')}</div></div>
    <div class="chart-container"><div class="chart-title">📈 月度进销对比</div><div class="bar-chart">${allMonths.length?allMonths.map(m=>`<div class="bar-col" style="gap:2px"><div class="bar-val" style="font-size:9px">${Math.round((monthSales[m]||0)/100)/10}k</div><div style="display:flex;gap:3px;align-items:flex-end;width:100%;height:130px"><div class="bar" style="height:${(monthPurchase[m]||0)/maxM*130}px;background:var(--cyan);flex:1" title="采购"></div><div class="bar" style="height:${(monthSales[m]||0)/maxM*130}px;background:var(--purple);flex:1" title="销售"></div></div><div class="bar-label">${m.slice(5)}</div></div>`).join(''):'<div style="color:var(--text3);text-align:center;width:100%;padding:40px 0;font-size:13px">暂无数据</div>'}</div>${allMonths.length?'<div style="display:flex;gap:16px;margin-top:8px;font-size:11px;color:var(--text3)"><span>🟦 采购</span><span>🟪 销售</span></div>':''}</div>
  `;

  // Low stock
  const lows = D.items.filter(i=>i.quantity<=i.minStock);
  document.getElementById('low-body').innerHTML = lows.map(i=>`<tr><td>${esc(i.name)}</td><td style="color:var(--red);font-weight:600">${i.quantity} ${esc(i.unit)}</td><td>${i.minStock}</td><td><button class="btn btn-sm btn-primary" onclick="go('purchase');setTimeout(()=>openPOModal('${i.id}'),100)">采购</button></td></tr>`).join('');
...
function renderInv() {
  ...
  body.innerHTML = f.map(i=>{
    ...
    const stL=st==='out'?'缺货':st==='low'?'偏低':'充足';
    return `<tr><td>${esc(i.name)}</td><td style="font-family:'JetBrains Mono',monospace;font-size:12px;color:var(--text3)">${esc(i.sku)}</td><td>${esc(i.category)}</td><td><strong>${i.quantity}</strong> ${esc(i.unit)}</td><td>${money(i.costPrice)}</td><td>${money(i.salePrice)}</td><td><span class="badge badge-${st}">● ${stL}</span></td><td><div class="actions-cell"><button class="action-btn" onclick="openItemModal('${i.id}')" title="编辑">✏️</button><button class="action-btn del" onclick="delItem('${i.id}')" title="删除">🗑️</button></div></td></tr>`;
  }).join('');
  // update filter
  document.getElementById('inv-cat').innerHTML = '<option value="">全部分类</option>'+D.categories.map(c=>`<option value="${c}" ${c===cat?'selected':''}>${c}</option>`).join('');
}
...
function openItemModal(id) {
  ...
  openM(i?'编辑商品':'新增商品', `
    ...
    <div class="form-group"><label class="form-label">商品名称 *</label><input class="form-input" id="fi-name" value="${esc(i?i.name:'')}"></div>
    <div class="form-row">
      <div class="form-group"><label class="form-label">编号/SKU</label><input class="form-input" id="fi-sku" value="${esc(i?i.sku:'SKU-'+String(D.items.length+1).padStart(4,'0'))}"></div>
      <div class="form-group"><label class="form-label">分类</label><select class="form-input" id="fi-cat">${catOpts}</select></div>
    </div>
    <div class="form-row-3">
      <div class="form-group"><label class="form-label">库存数量</label><input type="number" class="form-input" id="fi-qty" min="0" value="${i?i.quantity:0}"></div>
      <div class="form-group"><label class="form-label">成本价</label><input type="number" class="form-input" id="fi-cost" min="0" step="0.01" value="${i?i.costPrice:0}"></div>
      <div class="form-group"><label class="form-label">售价</label><input type="number" class="form-input" id="fi-sale" min="0" step="0.01" value="${i?i.salePrice:0}"></div>
    </div>
    <div class="form-row">
      <div class="form-group"><label class="form-label">单位</label><input class="form-input" id="fi-unit" value="${esc(i?i.unit:'个')}"></div>
      <div class="form-group"><label class="form-label">预警值</label><input type="number" class="form-input" id="fi-min" min="0" value="${i?i.minStock:10}"></div>
    </div>
    <div class="form-group"><label class="form-label">备注</label><input class="form-input" id="fi-note" value="${esc(i?i.note:'')}"></div>
  `, `<button class="btn" onclick="closeM()">取消</button><button class="btn btn-primary" onclick="saveItem()">保存</button>`);
}
...
function saveItem() {
  ...
  const data = {
    ...
    unit:document.getElementById('fi-unit').value.trim()||'个', minStock:Math.max(0,parseInt(document.getElementById('fi-min').value)||0),
    note:document.getElementById('fi-note').value.trim()
  };
  if(id) { const idx=D.items.findIndex(x=>x.id===id); if(idx>=0)D.items[idx]={...D.items[idx],...data}; addLog('edit',`编辑商品"${name}"`); toast('已更新'); }
  else { D.items.push({id:uid(),...data,createdAt:now()}); addLog('add',`新增商品"${name}"，库存${data.quantity}`); toast('已添加'); }
  save(); closeM(); refreshAll();
}

function delItem(id) { const i=D.items.find(x=>x.id===id); if(!i||!confirm(`删除"${i.name}"？`))return; D.items=D.items.filter(x=>x.id!==id); addLog('delete',`删除"${i.name}"`); save(); toast('已删除'); refreshAll(); }
...
function renderPO() {
  ...
  body.innerHTML=f.map(p=>{
    ...
    const stL=p.status==='done'?'已完成':p.status==='cancel'?'已取消':'待入库';
    return `<tr><td style="font-family:'JetBrains Mono',monospace;font-size:12px">${esc(p.no)}</td><td>${esc(p.supplierName||'-')}</td><td>${esc(it?it.name:'已删除')}</td><td>${p.qty}</td><td>${money(p.qty*p.price)}</td><td><span class="badge badge-${stCls}">${stL}</span></td><td style="font-size:12px;color:var(--text3)">${p.date}</td><td><div class="actions-cell">${p.status==='pending'?`<button class="btn btn-sm btn-green" onclick="completePO('${p.id}')">入库</button><button class="btn btn-sm btn-danger" onclick="cancelPO('${p.id}')">取消</button>`:''}</div></td></tr>`;
  }).join('');
}

function openPOModal(preItemId) {
  const itemOpts=D.items.map(i=>`<option value="${i.id}" ${i.id===preItemId?'selected':''}>${i.name}</option>`).join('');
  const supOpts='<option value="">无</option>'+D.suppliers.map(s=>`<option value="${s.id}">${s.name}</option>`).join('');
  openM('新建采购单',`
    <div class="form-group"><label class="form-label">商品 *</label><select class="form-input" id="fp-item">${itemOpts}</select></div>
    <div class="form-row">
      <div class="form-group"><label class="form-label">数量 *</label><input type="number" class="form-input" id="fp-qty" min="1" value="1"></div>
      <div class="form-group"><label class="form-label">采购单价</label><input type="number" class="form-input" id="fp-price" min="0" step="0.01" value="0"></div>
    </div>
    <div class="form-row">
      <div class="form-group"><label class="form-label">供应商</label><select class="form-input" id="fp-sup">${supOpts}</select></div>
      <div class="form-group"><label class="form-label">日期</label><input type="date" class="form-input" id="fp-date" value="${today()}"></div>
    </div>
    <div class="form-group"><label class="form-label">备注</label><input class="form-input" id="fp-note"></div>
  `,`<button class="btn" onclick="closeM()">取消</button><button class="btn btn-primary" onclick="savePO()">创建</button>`);
  // auto fill price
...
function savePO() {
  ...
  if(!itemId||qty<=0){toast('请选择商品和数量','error');return;}
  ...
  addLog('purchase',`新建采购单：${it?it.name:'?'} × ${qty}，单价${money(price)}`);
  save();closeM();toast('采购单已创建');renderPO();
}
...
function completePO(id) {
  ...
  p.status='done';
  addLog('purchase',`采购入库：${it?it.name:'?'} × ${p.qty}，${money(p.qty*p.price)}`);
  save();toast('已入库');refreshAll();
}
function cancelPO(id) { const p=D.purchases.find(x=>x.id===id);if(!p)return;p.status='cancel';addLog('purchase',`取消采购单${p.no}`);save();toast('已取消');renderPO(); }
...
function renderSO() {
  ...
  body.innerHTML=f.map(p=>{
    ...
    const stCls=p.status==='done'?'done':p.status==='cancel'?'cancel':'pending';
    const stL=p.status==='done'?'已完成':p.status==='cancel'?'已取消':'待出库';
    return `<tr><td style="font-family:'JetBrains Mono',monospace;font-size:12px">${esc(p.no)}</td><td>${esc(p.customerName||'-')}</td><td>${esc(it?it.name:'已删除')}</td><td>${p.qty}</td><td>${money(p.qty*p.price)}</td><td style="color:${profit>=0?'var(--green)':'var(--red)'}">${money(profit)}</td><td><span class="badge badge-${stCls}">${stL}</span></td><td style="font-size:12px;color:var(--text3)">${p.date}</td><td><div class="actions-cell">${p.status==='pending'?`<button class="btn btn-sm btn-green" onclick="completeSO('${p.id}')">出库</button><button class="btn btn-sm btn-danger" onclick="cancelSO('${p.id}')">取消</button>`:''}</div></td></tr>`;
  }).join('');
}

function openSOModal() {
  const itemOpts=D.items.map(i=>`<option value="${i.id}">${i.name} (库存:${i.quantity})</option>`).join('');
  const custOpts='<option value="">散客</option>'+D.customers.map(c=>`<option value="${c.id}">${c.name}</option>`).join('');
  openM('新建销售单',`
    <div class="form-group"><label class="form-label">商品 *</label><select class="form-input" id="fs-item">${itemOpts}</select></div>
    <div class="form-row">
      <div class="form-group"><label class="form-label">数量 *</label><input type="number" class="form-input" id="fs-qty" min="1" value="1"></div>
      <div class="form-group"><label class="form-label">销售单价</label><input type="number" class="form-input" id="fs-price" min="0" step="0.01" value="0"></div>
    </div>
    <div class="form-row">
      <div class="form-group"><label class="form-label">客户</label><select class="form-input" id="fs-cust">${custOpts}</select></div>
      <div class="form-group"><label class="form-label">日期</label><input type="date" class="form-input" id="fs-date" value="${today()}"></div>
    </div>
    <div class="form-group"><label class="form-label">备注</label><input class="form-input" id="fs-note"></div>
  `,`<button class="btn" onclick="closeM()">取消</button><button class="btn btn-primary" onclick="saveSO()">创建</button>`);
...
function saveSO() {
  ...
  D.sales.push({id:uid(),no:'SO-'+Date.now().toString().slice(-8),itemId,qty,price,customerId:custId,customerName:cust?cust.name:'散客',date:document.getElementById('fs-date').value,note:document.getElementById('fs-note').value,status:'pending'});
  addLog('sale',`新建销售单：${it?it.name:'?'} × ${qty}，单价${money(price)}`);
  save();closeM();toast('销售单已创建');renderSO();
}

function completeSO(id) {
  const p=D.sales.find(x=>x.id===id); if(!p)return;
  const it=D.items.find(i=>i.id===p.itemId);
  if(it) { if(it.quantity<p.qty){toast('库存不足！','error');return;} it.quantity-=p.qty; }
  p.status='done';
  addLog('sale',`销售出库：${it?it.name:'?'} × ${p.qty}，${money(p.qty*p.price)}`);
  save();toast('已出库');refreshAll();
}
function cancelSO(id) { const p=D.sales.find(x=>x.id===id);if(!p)return;p.status='cancel';addLog('sale',`取消销售单${p.no}`);save();toast('已取消');renderSO(); }

// ===== QUICK IO =====
function renderIO() {
  document.getElementById('io-item').innerHTML = D.items.length?D.items.map(i=>`<option value="${i.id}">${i.name} (${i.quantity} ${i.unit})</option>`).join(''):'<option value="">暂无商品</option>';
}
function doIO() {
  ...
  if(!id){toast('请先添加商品','error');return;} if(qty<=0){toast('数量须>0','error');return;}
  const i=D.items.find(x=>x.id===id);if(!i)return;
  if(type==='out'&&i.quantity<qty){toast('库存不足','error');return;}
  if(type==='in')i.quantity+=qty; else i.quantity-=qty;
  addLog(type,`${type==='in'?'入库':'出库'}"${i.name}" × ${qty}${note?' ('+note+')':''}，余${i.quantity}`);
  save();toast(`${type==='in'?'入库':'出库'}成功`);document.getElementById('io-qty').value=1;document.getElementById('io-note').value='';renderIO();refreshAll();
}
...
function renderSup() {
  ...
  body.innerHTML=D.suppliers.map(s=>{
    ...
    return `<tr><td><strong>${esc(s.name)}</strong></td><td>${esc(s.contact)}</td><td>${esc(s.phone)}</td><td>${esc(s.address)}</td><td>${pos.length}</td><td>${money(pos.reduce((a,p)=>a+p.qty*p.price,0))}</td><td><div class="actions-cell"><button class="action-btn" onclick="openSupModal('${s.id}')">✏️</button><button class="action-btn del" onclick="delSup('${s.id}')">🗑️</button></div></td></tr>`;
  }).join('');
}
...
function openSupModal(id) {
  ...
  openM(s?'编辑供应商':'新增供应商',`
    ...
    <div class="form-group"><label class="form-label">名称 *</label><input class="form-input" id="fsu-name" value="${esc(s?s.name:'')}"></div>
    <div class="form-row"><div class="form-group"><label class="form-label">联系人</label><input class="form-input" id="fsu-contact" value="${esc(s?s.contact:'')}"></div><div class="form-group"><label class="form-label">电话</label><input class="form-input" id="fsu-phone" value="${esc(s?s.phone:'')}"></div></div>
    <div class="form-group"><label class="form-label">地址</label><input class="form-input" id="fsu-addr" value="${esc(s?s.address:'')}"></div>
    <div class="form-group"><label class="form-label">备注</label><input class="form-input" id="fsu-note" value="${esc(s?s.note:'')}"></div>
  `,`<button class="btn" onclick="closeM()">取消</button><button class="btn btn-primary" onclick="saveSup()">保存</button>`);
}
function saveSup() {
  const id=document.getElementById('fsu-id').value,name=document.getElementById('fsu-name').value.trim();if(!name){toast('请输入名称','error');return;}
  ...
  if(id){const idx=D.suppliers.findIndex(x=>x.id===id);if(idx>=0)D.suppliers[idx]={...D.suppliers[idx],...data};toast('已更新');}
  else{D.suppliers.push({id:uid(),...data});toast('已添加');}
  save();closeM();renderSup();
}
function delSup(id){const s=D.suppliers.find(x=>x.id===id);if(!s||!confirm(`删除"${s.name}"？`))return;D.suppliers=D.suppliers.filter(x=>x.id!==id);save();toast('已删除');renderSup();}
...
function renderCust() {
  ...
  body.innerHTML=D.customers.map(c=>{
    ...
    return `<tr><td><strong>${esc(c.name)}</strong></td><td>${esc(c.contact)}</td><td>${esc(c.phone)}</td><td>${esc(c.address)}</td><td>${sos.length}</td><td>${money(sos.reduce((a,p)=>a+p.qty*p.price,0))}</td><td><div class="actions-cell"><button class="action-btn" onclick="openCustModal('${c.id}')">✏️</button><button class="action-btn del" onclick="delCust('${c.id}')">🗑️</button></div></td></tr>`;
  }).join('');
}
function openCustModal(id) {
  ...
  openM(c?'编辑客户':'新增客户',`
    ...
    <div class="form-group"><label class="form-label">名称 *</label><input class="form-input" id="fc-name" value="${esc(c?c.name:'')}"></div>
    <div class="form-row"><div class="form-group"><label class="form-label">联系人</label><input class="form-input" id="fc-contact" value="${esc(c?c.contact:'')}"></div><div class="form-group"><label class="form-label">电话</label><input class="form-input" id="fc-phone" value="${esc(c?c.phone:'')}"></div></div>
    <div class="form-group"><label class="form-label">地址</label><input class="form-input" id="fc-addr" value="${esc(c?c.address:'')}"></div>
  `,`<button class="btn" onclick="closeM()">取消</button><button class="btn btn-primary" onclick="saveCust()">保存</button>`);
}
function saveCust() {
  const id=document.getElementById('fc-id').value,name=document.getElementById('fc-name').value.trim();if(!name){toast('请输入名称','error');return;}
  ...
  if(id){const idx=D.customers.findIndex(x=>x.id===id);if(idx>=0)D.customers[idx]={...D.customers[idx],...data};toast('已更新');}
  else{D.customers.push({id:uid(),...data});toast('已添加');}
  save();closeM();renderCust();
}
function delCust(id){const c=D.customers.find(x=>x.id===id);if(!c||!confirm(`删除"${c.name}"？`))return;D.customers=D.customers.filter(x=>x.id!==id);save();toast('已删除');renderCust();}
...
function doStocktake() {
  ...
  document.querySelectorAll('[data-st-id]').forEach(inp=>{
    ...
    if(newQty!==it.quantity){
      addLog('stocktake',`盘点"${it.name}"：${it.quantity}→${newQty}（差异${newQty-it.quantity>0?'+':''}${newQty-it.quantity}）`);
      it.quantity=newQty; changes++;
    }
  });
  save();
  if(changes) toast(`盘点完成，${changes}项已更新`); else toast('无差异');
  refreshAll();
}
...
function renderFinance() {
  ...
  document.getElementById('fin-summary').innerHTML = [
    {l:'采购总额',v:money(totalPurchase),c:'var(--cyan)'},
    {l:'销售总额',v:money(totalSales),c:'var(--purple)'},
    {l:'毛利润',v:money(totalProfit),c:totalProfit>=0?'var(--green)':'var(--red)'},
    {l:'利润率',v:profitRate+'%',c:'var(--yellow)'},
...
function clearLog(){if(!confirm('确定清空？'))return;D.logs=[];save();renderLog();toast('已清空');}

// ===== CATEGORIES =====
function renderCat() {
  document.getElementById('cat-body').innerHTML=D.categories.map(c=>{
    const cnt=D.items.filter(i=>i.category===c).length;
    return `<tr><td><strong>${esc(c)}</strong></td><td>${cnt}</td><td><button class="action-btn del" onclick="delCat('${esc(c)}')">🗑️</button></td></tr>`;
  }).join('');
}
function addCat(){const inp=document.getElementById('new-cat'),n=inp.value.trim();if(!n)return;if(D.categories.includes(n)){toast('已存在','error');return;}D.categories.push(n);save();inp.value='';renderCat();toast('已添加');}
function delCat(n){if(!confirm(`删除分类"${n}"？`))return;D.categories=D.categories.filter(c=>c!==n);save();renderCat();toast('已删除');}

// ===== EXPORT / IMPORT =====
function exportData(){const blob=new Blob([JSON.stringify(D,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='进销存数据_'+today()+'.json';a.click();toast('已导出');}
function importData(){document.getElementById('imp-file').click();}
function handleImp(e){const file=e.target.files[0];if(!file)return;const r=new FileReader();r.onload=function(ev){try{const d=JSON.parse(ev.target.result);KEYS.forEach(k=>{if(d[k])D[k]=d[k]});save();refreshAll();toast('导入成功');}catch{toast('文件格式错误','error');}};r.readAsText(file);e.target.value='';}
